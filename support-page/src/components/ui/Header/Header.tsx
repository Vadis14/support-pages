        ...
        <header className={styles.header}>
            {HeaderContainer.map((item) => {
                switch(item.kind) {
                    ...
                    case 'select':
                        return <Select options={item.options.map((option) => ({value: option.value, textKey: option.textKey}))} name={item.name} value={item.value} variant={item.variant} onChange={handleChangeLanguage} />
                }
            })}
        </header>
    )
