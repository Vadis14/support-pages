import { keysFromSelector } from "./chunk-6MGD5ELF.js";
  ...
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.development.js"(
    ...
    (function () {
      ...
      var React3 = require_react(),
        ...
        useState2 = React3.useState,
        useEffect2 = React3.useEffect,
        useLayoutEffect = React3.useLayoutEffect,
    ...
    return astNodes.reduce((mem, node, i2) => {
      ...
      if (node.type === "tag") {
        ...
        const isKnownComponent =
          isObject(knownComponentsMap) &&
          Object.hasOwnProperty.call(knownComponentsMap, node.name);
        if (isString(child)) {
...
function Trans({
  ...
  const mergedTOptions = transDefaultProps?.tOptions
    ...
    : tOptions;
  const mergedShouldUnescape =
    shouldUnescape ?? transDefaultProps?.shouldUnescape;
  ...
  const defaultValue =
    ...
    (typeof i18nKey === "function" ? keysFromSelector(i18nKey) : i18nKey);
  ...
  if (
    valuesFromChildren &&
    typeof valuesFromChildren.count === "number" &&
    count === void 0
  ) {
  ...
  const interpolationOverride =
    values ||
    (count !== void 0 && !i18n.options?.interpolation?.alwaysFormat) ||
    !children
...
var useTranslation = (ns, props = {}) => {
  ...
  const unstableNamespaces = isString(nsOrContext)
    ? [nsOrContext]
    ...
    (callback) => {
      ...
      return () => {
        ...
        if (bindI18nStore)
          bindI18nStore
            .split(" ")
            .forEach((e2) => i18n.store.off(e2, wrappedCallback));
      };
    },
    [i18n, i18nOptions],
  );
  ...
  const { t: t2, ready } = (0, import_shim.useSyncExternalStore)(
    subscribe,
    getSnapshot,
    getSnapshot,
  );
...
var withTranslation = (ns, options = {}) =>
  function Extend(WrappedComponent) {
    ...
    return options.withRef
      ? (0, import_react8.forwardRef)(forwardRef)
      : I18nextWithTranslation;
  };
